...
def match_with_odds_data(nfl_games: pd.DataFrame, nfl_pbp_stats: pd.DataFrame,
    ...
    nfl_combined = nfl_games.merge(nfl_pbp_stats, on=['game_id', 'home_team', 'away_team'], how='left')

    # Get unique games from odds data
    odds_games = odds_df.groupby(['game_id', 'home_team', 'away_team', 'commence_time']).first().reset_index()
...
def collect_and_save_nfl_data(odds_filepath: str, output_filepath: str,
    ...
    nfl_feature_cols = [col for col in combined_df.columns if col.startswith(('home_', 'away_'))
