...
def plot_model_comparison(
    results_df: pd.DataFrame, metric: str = "accuracy", save_path: Optional[str] = None
) -> Figure:
    """
    ...
    ax.set_title(
        f'Model Comparison - {metric.replace("_", " ").title()}', fontsize=14, fontweight="bold"
    )
    ax.grid(axis="y", alpha=0.3)

    # Add mean line
    means = [results_df[results_df["model"] == model][metric].mean() for model in models]
...
def plot_prediction_distribution(
    predictions: Dict[str, np.ndarray], save_path: Optional[str] = None
) -> Figure:
    """
...
def plot_arbitrage_opportunities(arb_df: pd.DataFrame, save_path: Optional[str] = None) -> Figure:
    ...
    if len(arb_df) == 0:
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.text(0.5, 0.5, "No Arbitrage Opportunities Found", ha="center", va="center", fontsize=16)
        ax.set_xlim([0, 1])
        ax.set_ylim([0, 1])
        ax.axis("off")
        return fig
    ...
    game_labels = [
        f"{row['away_team'][:10]} @ {row['home_team'][:10]}" for _, row in top_arb.iterrows()
    ]

    ax2.barh(
        range(len(top_arb)),
        top_arb["arbitrage_roi"].values,
        color=PROFIT_COLOR,
        alpha=0.75,
        edgecolor="black",
        linewidth=1.2,
...
def plot_metrics_heatmap(results_df: pd.DataFrame, save_path: Optional[str] = None) -> Figure:
    ...
    for idx, metric in enumerate(available_metrics):
        pivot_data = results_df.pivot_table(
            values=metric, index="model", columns="fold", aggfunc="mean"
        )
        ...
        axes[idx].set_title(
            f'{metric.replace("_", " ").title()} by Model and Fold', fontsize=12, fontweight="bold"
        )
...
def plot_strategy_comparison(
    ...
    Returns:
        Matplotlib figure
    """
    data = _prepare_strategy_data(fixed_roi_results, kelly_roi_results, markowitz_roi_results)
...
def plot_strategy_roi_comparison(
    fixed_roi_results: Dict[str, Dict],
    kelly_roi_results: Dict[str, Dict],
    markowitz_roi_results: Dict[str, Dict],
    save_path: Optional[str] = None,
...
def plot_strategy_profit_comparison(
    fixed_roi_results: Dict[str, Dict],
    kelly_roi_results: Dict[str, Dict],
    markowitz_roi_results: Dict[str, Dict],
    save_path: Optional[str] = None,
...
def plot_strategy_bet_frequency(
    fixed_roi_results: Dict[str, Dict],
    kelly_roi_results: Dict[str, Dict],
    markowitz_roi_results: Dict[str, Dict],
    save_path: Optional[str] = None,
...
def plot_strategy_best_strategy_counts(
    fixed_roi_results: Dict[str, Dict],
    kelly_roi_results: Dict[str, Dict],
    markowitz_roi_results: Dict[str, Dict],
    save_path: Optional[str] = None,
